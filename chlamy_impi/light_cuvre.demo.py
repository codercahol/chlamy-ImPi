...
Fv = Fm - F0
QEY = Fv / Fm
nan_or_inf = lambda x: torch.logical_or(torch.isinf(x), torch.isnan(x))
QEY[nan_or_inf(QEY)] = 0
sig_pix_cnt = torch.sum(QEY > 0, dim=(2,3))
flur =  torch.sum(QEY, dim = (2,3))
QEY_mean = flur / sig_pix_cnt
QEY_mean[nan_or_inf(QEY_mean)] = 0
...
#sns.set_theme()
sns.histplot(data=QEY_mean.view(-1), kde=True); plt.title("mean Fv/Fm at t={}".format(t))

# %% tabulate light Curve QEYs

mat = np.zeros((NUM_ROWS * NUM_COLUMNS, NUM_TIMESTEPS//2))
for T in range(NUM_TIMESTEPS//2):
    ...
    QEY_mean = flur / sig_pix_cnt
    QEY_mean[nan_or_inf(QEY_mean)] = 0
    mat[:,T] = QEY_mean.view(-1).numpy()

# %% plot ze data
sns.stripplot(data=mat); plt.axhline(y=0.0, color="black", linestyle="--")
