...
def compute_all_y2_averaged(img_array, mask_array) -> np.array:
    ...
    assert np.nanmin(y2_array_means) > -1.0, f"minimum Y2 bound failed: {np.nanmin(y2_array_means)}"

    # Check that the average Y2 of all wells at each time step is positive
    for i in range(num_steps):
        assert np.nanmean(y2_array_means[:, :, i]) > 0.0, f"Y2 mean is negative at time step {i}: {np.nanmean(y2_array_means[:, :, i])}, \n {y2_array_means[:, :, i]}"
