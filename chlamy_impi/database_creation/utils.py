...
def parse_name(f):
    ...
    assert time_regime in {"30s-30s", "1min-1min", "10min-10min", "2h-2h", "20h ML", "20h HL"}, f"{time_regime}, {f}"
...
def compute_measurement_times(meta_df: pd.DataFrame) -> list[datetime.datetime]:
    ...
    """
    meta_df["Datetime"] = meta_df[["Date", "Time"]].apply(lambda x: pd.to_datetime(x["Date"]) + pd.to_timedelta(x["Time"]), axis=1)
